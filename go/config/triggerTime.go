...
解釈できない値では Job にエラーを持たせるだけで Run() も失敗する。
設定ミスをそのまま渡すと「そのジョブが二度と発火しない」という形の
デグレになるため、scheduler.parseTime と同じ規則でここで先に弾き、
既定値に倒したうえでログへ警告を残す。

なお service.go 側では jobRegistry が Run() の戻り値を検査して
登録失敗を起動時にSlack通知する（F30-2）。本関数はその手前で
設定由来の失敗そのものを起こさせないための第一の防波堤にあたる。
