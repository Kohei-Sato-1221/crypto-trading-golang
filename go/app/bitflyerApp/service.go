...
func StartBfService() {
	...
	if !config.Config.IsTest {
		scheduler.Every().Day().At("19:15").Run(wrapJob(buyingBTCJobEveryDay))
		scheduler.Every().Day().At("19:15").Run(wrapJob(buyingETHJobEveryDay))
		...
		scheduler.Every().Day().At("19:15").Run(wrapJob(buyingBTCJobLTP5t5Sun))
