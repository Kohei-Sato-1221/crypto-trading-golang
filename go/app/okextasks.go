...
func StartOKEXService(exchange string) {
	...
	buyingJob01 := func() {
		...
		price := utils.RoundDecimal(utils.STf(ticker.Ltp)*0.4 + utils.STf(ticker.Low)*0.6)
	...
	buyingJob02 := func() {
		ticker, _ := apiClient.GetOkexTicker("EOS-USDT")
		price := utils.RoundDecimal(utils.STf(ticker.Ltp)*0.1 + utils.STf(ticker.Low)*0.9)
		log.Printf("#### EOS-USDT price:%v ", price)
		placeOkexBuyOrder("EOS-USDT", 1, price, apiClient, slackClient)
	}
	...
	buyingJob03 := func() {
		ticker, _ := apiClient.GetOkexTicker("EOS-USDT")
		price := utils.RoundDecimal(utils.STf(ticker.Ltp) * 0.975)
		log.Printf("#### EOS-USDT price:%v ", price)
		placeOkexBuyOrder("EOS-USDT", 1, price, apiClient, slackClient)
	}
	...
	buyingJob04 := func() {
		ticker, _ := apiClient.GetOkexTicker("EOS-USDT")
		price := utils.RoundDecimal(utils.STf(ticker.Ltp) * 0.985)
		log.Printf("#### EOS-USDT price:%v ", price)
		placeOkexBuyOrder("EOS-USDT", 1, price, apiClient, slackClient)
	}
	...
	buyingOKBJob01 := func() {
		...
		price := utils.RoundDecimal(utils.STf(ticker.Ltp)*0.3 + utils.STf(ticker.Low)*0.7)
	...
	buyingBTCJob03 := func() {
		ticker, _ := apiClient.GetOkexTicker("BTC-USDT")
		price := utils.RoundDecimal(utils.STf(ticker.Ltp) * 0.997)
		log.Printf("#### BTC-USDT price:%v ", price)
	...
	buyingETHJob01 := func() {
		...
		price := utils.RoundDecimal(utils.STf(ticker.Ltp) * 0.995)
	...
	buyingETHJob02 := func() {
		ticker, _ := apiClient.GetOkexTicker("ETH-USDT")
		price := utils.RoundDecimal(utils.STf(ticker.Ltp) * 0.98)
		log.Printf("#### ETH-USDT price:%v ", price)
	...
	buyingETHJob03 := func() {
		ticker, _ := apiClient.GetOkexTicker("ETH-USDT")
		price := utils.RoundDecimal(utils.STf(ticker.Ltp) * 0.97)
		log.Printf("#### ETH-USDT price:%v ", price)
	...
	cancelBuyOrderJob := func() {
		...
		cancelCriteria := time.Now().AddDate(0, 0, utils.OkexCancelCriteria)
...
func GetAvailableBalance(currency string, apiClient *okex.APIClient) float64 {
	...
	if balance == nil {
		...
		return utils.STf(balance.Available)
	}
}
...
func placeOkexOrder(side, buyOrderID, pair string, size, price float64, apiClient *okex.APIClient, slackClient *slack.APIClient) (orderId string, fixedSize float64) {
	...
	order := &okex.Order{
		...
		InstrumentID: pair,
		OrderType:    "0",
		Price:        utils.FTs(utils.RoundDecimal(price)),
		Size:         utils.FTs(size),
	}
	...
	if res == nil {
		...
		text := getErrorMessageForSlack(
			...
			side,
			pair,
			utils.FTs(utils.RoundDecimal(price)),
			utils.FTs(size))

		if res.ErrorCode == "33017" {
			if side == "sell" {
				fixedSize = size * 0.9
				order.Size = utils.FTs(fixedSize)
				res, err = apiClient.PlaceOrder(order)
				if res.ErrorCode == "33017" {
