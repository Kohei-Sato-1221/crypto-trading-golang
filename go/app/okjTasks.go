...
func StartOKJService(exchange string) {
	...
	buyingJob02 := func() {
		...
		for _, price := range prices {
			...
			placeOkexBuyOrder("ETH-JPY", 0.015, price, apiClient, slackClient)
		}
	}
	...
	cancelBuyOrderJob := func() {
		...
		cancelCriteria := time.Now().AddDate(0, 0, utils.OkjCancelCriteria)
...
func getBuyPrices(low, last float64, numOfPrices int) []float64 {
	roundedLow := utils.RoundDecimal(low * 1.005)
	roundedLast := utils.RoundDecimal(last * 0.995)

	diff := (roundedLast - roundedLow) / float64(numOfPrices)
	prices := make([]float64, 0, numOfPrices)

	for i := 0; i < numOfPrices; i++ {
		prices = append(prices, utils.RoundDecimal(roundedLow+(diff*float64(i))))
	}
