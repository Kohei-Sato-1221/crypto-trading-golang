...
func (e *OrderEvent) BuyOrder() error {
	cmd1, err := AppDB.Prepare("INSERT INTO buy_orders (order_id, product_code, side, price, size, exchange, strategy, remarks) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")
	if err != nil {
...
func FilledCheck(productCode string) ([]string, error) {
	cmd, _ := AppDB.Prepare(`SELECT order_id FROM buy_orders WHERE status = 'UNFILLED' and order_id != '' and product_code = ? union SELECT order_id FROM sell_orders WHERE status = 'UNFILLED' and order_id != '' and product_code = ?`)
...
func UpdateFilledOrder(order_id string) error {
	...
	cmd2, _ := AppDB.Prepare(`update sell_orders set status = 'FILLED' where order_id = ?`)
...
func UpdateCancelledBuyOrder(order_id string) error {
	cmd, _ := AppDB.Prepare(`update buy_orders set status = 'CANCELLED' where order_id = ?`)
...
func SyncBuyOrders(events *[]OrderEvent) {
	for _, e := range *events {
		...
		if cnt == 0 {
			...
			state := "INSERT INTO buy_orders (order_id, product_code, side, price, size, exchange, status, remarks) VALUES (" +
				"'" + e.OrderID + "'," +
				"'" + e.ProductCode + "'," +
				"'" + e.Side + "'," +
