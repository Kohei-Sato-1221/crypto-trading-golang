...
func FilledCheckWithSellOrder() []Idprice {
	cmd := `SELECT orderid, price, product_code, size, exchange FROM buy_orders WHERE filled = 1 and orderid != '';`
	rows, err := DbConnection.Query(cmd)
	if err != nil {
		return nil
	}
...
func SyncBuyOrders(events *[]OrderEvent) {
	...
	cmd2 := fmt.Sprintf("INSERT INTO buy_orders (orderid, time, product_code, side, price, size, exchange, filled) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")
	for _, e := range *events {
		...
		if cnt == 0 {
			_, err := DbConnection.Exec(cmd2, e.OrderId, e.Time.Format(time.RFC3339), e.ProductCode, e.Side, e.Price, e.Size, e.Exchange, e.Filled)
			if err != nil {
