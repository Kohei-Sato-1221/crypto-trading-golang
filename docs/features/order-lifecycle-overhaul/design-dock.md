...
## 11. スコープ外とした事項（Phase 3 レビュー後の確定）

### 11.1 F27: 売り注文の部分約定に対する損益按分（対応しない）

**指摘内容**: 部分約定した売り注文をローリングすると旧レコードが `CANCELLED` になり損益集計（`a.parentid = b.order_id AND a.status='FILLED'`）から外れるため、既に約定した数量ぶんの売却額が計上されない。損益レポートが実態より過小に出続ける。

**判断: 対応しない（スコープ外）。** 理由は以下の3点。

1. **現状は安全側に倒れている。** ずれは常に過小方向であり、利益を過大に見せることはない。実損も発生しない
2. **部分約定はこれまで1件も発生していない。** 発生頻度が極めて低い事象に対する先回りの投資になる
3. **恒久対応の規模が他の指摘と桁違い。** 正しく按分するには買い注文側にも数量を分割して持たせる必要があり、`buy_orders` のデータモデルと `placeSellOrder` / `GetExpectedHoldings` / 損益クエリの前提を同時に変える設計変更になる

**代わりに実施したこと**（本ブランチの G1〜G3）:

| | 内容 |
|---|---|
| **G1** | `rolloverOrderSnapshot` に `averagePrice` を追加し、`lookupRolloverOrder` が `average_price` を保持するようにした。**キャンセルした注文は取引所APIから即座に消える**ため、ここで拾わないと平均約定価格は二度と取得できず、手動対応が原理的に不可能になる |
| **G2** | パターン①（残数量が最小取引単位以上）・パターン②（未満）の双方のSlack通知に、「システムが部分約定の損益計算に対応していないこと」「損益レポートが実態より過小に出ること」「正確な損益には手動対応が必要なこと」と、手動対応に必要な実値一式（親買い注文ID / 売り注文ID / product_code / 平均約定価格 / 約定数量 / 元size / 残数量）を追記した |
| **G3** | ルート `CLAUDE.md` に「売り注文の部分約定が起きたときの対応」を追記し、実行可能な SQL 付きの手動対応手順・検証クエリ・パターン②の対応方針を明文化した。手順の正しさは `go/tests/partial_fill_manual_fix_test.go` でローカル PostgreSQL に対して検証済み |

**手動対応で注意すべき点**: 「約定ぶんの売り注文を1行 INSERT するだけ」では直らない。同一 `parentid` に `FILLED` の売りが2行並ぶと買いコスト（`b.price * b.size`）が二重に引かれ、対応前より悪化する（ローカル検証: `-4,747.87` → `-14,626.99`。正しい値は `+356.51`）。買い注文側も同じ比率で分割する必要がある。

### 11.2 その他スコープ外の指摘

- **F1**: `scripts/migration/*.sh` への DB パスワード平文コミット（§10.2 に既述。認証情報のローテーションを別途推奨）
- **F29**: 取引所APIレスポンス全文のログ出力
